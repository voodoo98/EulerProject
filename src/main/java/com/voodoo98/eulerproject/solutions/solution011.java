...
import com.voodoo98.eulerproject.matrix.Matrix;

public class solution011 {

	public static void main(final String[] args) {
		final String matrixStr = "08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08 49 49 99 40 17 81 18 57 60 87 17 40 98 43 69 48 04 56 62 00"
		...
		final Matrix matrix = new Matrix(matrixStr, 20);
		checkMatrix(matrix.convert());
	}

	private static void checkMatrix(final int[][] matrix) {
		long max = 0;
		for (int i = 0; i < matrix.length; i++) {
			for (int j = 0; j < matrix.length; j++) {
				final long checkStarForm = checkStarForm(matrix, i, j);
				if (max < checkStarForm) {
					max = checkStarForm;
				}
			}
		}
	...
	private static long checkStarForm(final int[][] matrix, final int i, final int j) {
		...
		for (int k = 0; k < 4; k++) {
			if (j + 3 < dimension) {
				sumRight *= matrix[i][j + k];
			}
			if (i + 3 < dimension && j + 3 < dimension) {
				sumDiagonal1 *= matrix[i + k][j + k];
			}
			if (i + 3 < dimension && 3 < j) {
				sumDiagonal2 *= matrix[i + k][j - k];
			}
			if (i + 3 < dimension) {
				sumDown *= matrix[i + k][j];
			}
		}
		return Math.max(sumDiagonal2, Math.max(Math.max(sumDiagonal1, sumDown), sumRight));
	}
}
